...
RunalyzeLeaflet.Routes = (function($, parent, Math){
	...
	function onMove(e) {
		if (mouseover !== false) {
			// show tooltip if the mouse is moved on the new position
			txt = '';
			if(objects[mouseover].segmentsInfo.length) {
				txt = getTooltipTextSegment(getSegmentsInfoAt(e.latlng));
			} else if(objects[mouseover].tooltip) {
				txt = objects[mouseover].tooltip;
			}
	...
	function onOver(e) {
		...
		// here the tooltip is activated when the mouse is over
		if (objects[this.id].segmentsInfo.length) {
			// tooltip for the segments
			mouseover = this.id;
			txt = getTooltipTextSegment(getSegmentsInfoAt(e.latlng));
		} else if(objects[this.id].tooltip) {
			// #TSC tooltip on the whole route
			mouseover = this.id;
			txt = objects[this.id].tooltip;
		}
	...
	// get a JSON info structure (f.e. the distance and time)
	...
	// create the tooltip text for the "selected" SegmentsInfo
	function getTooltipTextSegment(info) {
		if (!info.length)
			return;
